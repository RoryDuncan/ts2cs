...
import { InterfaceDeclaration, PropertySignature, MethodSignature } from "ts-morph";
...
export function transpileInterface(
  ...
  const typeParamStr = typeParams.length > 0 ? `<${typeParams.map((tp) => tp.getName()).join(", ")}>` : "";
 ...
 */
function transpileInterfaceProperty(prop: PropertySignature, mappings: ResolvedTypeMappings, indent: string): string {
 ...
 */
function transpileInterfaceMethod(method: MethodSignature, mappings: ResolvedTypeMappings, indent: string): string {
